...
def _assert_can_assign(actor: User, target_role: str) -> None:
    """Chỉ root được gán vai manager hoặc root; manager chỉ được gán vai staff."""
    if actor.role != "root" and target_role != "staff":
        raise HTTPException(status.HTTP_403_FORBIDDEN, "chỉ root được gán vai manager hoặc root")


def _assert_can_modify(actor: User, target: User) -> None:
    """Manager chỉ được quản lý tài khoản staff; root quản lý mọi tài khoản."""
    if actor.role != "root" and target.role != "staff":
        raise HTTPException(status.HTTP_403_FORBIDDEN, "manager chỉ được quản lý tài khoản staff")
