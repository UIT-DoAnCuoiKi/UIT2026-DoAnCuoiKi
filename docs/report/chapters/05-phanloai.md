...
Cả 2 kiến trúc hội tụ nhanh và ổn định. Bài toán 3 lớp dễ hơn hẳn bài toán 12 lớp đã thử trước đó: accuracy epoch 1 đã trên 0,80, so với khoảng 0,4-0,6 ở bản 12 lớp. Khoảng cách accuracy trên tập test giữa ResNet18 và MobileNetV3-Small cũng thu hẹp, từ 6,7 điểm phần trăm ở bài toán 12 lớp (0,768 so với 0,701) còn 0,1 điểm (0,8997 so với 0,9007).
...
Lần huấn luyện đầu chỉ đúng 1/13 (mục 5.1), nên kết quả này cho thấy trộn phong cách ảnh đã sửa đúng nguyên nhân gốc. Khi nghi model học tắt, cách kiểm chứng đáng tin là một tập OOD nhỏ nhưng chắc chắn khác nguồn với tập train. Chia lại một tập test lớn hơn từ cùng nguồn thì không kiểm chứng được gì.
...
*(CPU inference ở bảng này đo trên máy train. Trên Raspberry Pi 5, ResNet18 mất khoảng 130 ms mỗi ảnh ở 1 luồng, MobileNetV3-Small khoảng 12,5 ms, xem mục 5.6.)*
...
**Hướng tiếp theo:** thu thập dữ liệu xe khách đúng góc camera cổng để huấn luyện nốt lớp `bus`; tìm hoặc thu thập dữ liệu đúng góc camera cổng bãi xe cho bước định vị (YOLO); thử INT8 quantization nếu cần nhanh hơn nữa trên Raspberry Pi 5 (bước kiểu dáng đã chuyển sang MobileNetV3-Small ONNX, mục 5.6).
...
| `src/ml/weights/vehicle-style-resnet18.pt` / `.onnx` | Model kiểu dáng, bản ResNet18 (44,8 MB) |
| `src/ml/weights/vehicle-style-mobilenet_v3_small.pt` / `.onnx` | Model kiểu dáng, bản MobileNetV3-Small (6,1 MB), mặc định dùng trong pipeline; bản ONNX khớp `.pt` 987/987 ảnh test (`experiments/style_mobilenet_onnx_parity.json`) |
...
| `src/ml/run_pi_benchmark.sh`, `pi_power_monitor.py`, `summarize_pi_benchmark.py` | Đo trên Raspberry Pi 5: chạy ma trận số luồng, ghi log điện năng, ghép log thành bảng; kết quả xem bằng `notebooks/benchmark-raspberry-pi5.ipynb` |
...
Ép mỗi model dùng cùng 1 số luồng cố định qua biến môi trường `ML_INTRAOP_THREADS` (đặt `intra_op_num_threads`/`inter_op_num_threads` cho từng session ONNX và `torch.set_num_threads()` cho nhánh torch) giải quyết vấn đề này. Mặc định trong code là 1 luồng/model, giá trị an toàn trên máy nhiều lõi. Bản trước của mục này dự đoán trên Raspberry Pi chia nhiều luồng còn dễ phản tác dụng hơn; số đo thật trên Pi 5 (4 lõi) cho thấy ngược lại: 4 luồng nhanh nhất, 495,3 ms mỗi lượt so với 813,9 ms ở 1 luồng. Khi triển khai trên Pi đặt `ML_INTRAOP_THREADS=4`.
...
Vì vậy, thay vì chốt cứng một lựa chọn, `CoarseVehicleDetector` hỗ trợ cả 2 backend, chọn qua đường dẫn trọng số (đuôi `.onnx` dùng onnxruntime thuần, còn lại dùng `.pt` qua ultralytics), cùng quy ước với `PlateDetector` của module biển số. Mặc định vẫn là `.pt` (nhanh hơn trên CPU x86 thường gặp ở máy dev/server); đổi sang `.onnx` qua biến môi trường `ML_COARSE_WEIGHTS` khi triển khai một thiết bị không muốn cài torch. Trên Raspberry Pi 5 bản `.onnx` cũng chậm hơn: toàn pipeline 1.067,4 ms so với 813,9 ms ở 1 luồng, 535,0 ms so với 495,3 ms ở 4 luồng (mục bên dưới). Cách chia này giữ framework huấn luyện (ultralytics/torch) ở máy phát triển: xuất model sang định dạng trung gian (ONNX), rồi chạy trên thiết bị bằng runtime suy luận nhỏ gọn với tiền/hậu xử lý tự viết, không phải cài cả framework huấn luyện lên thiết bị. Riêng phần tiền xử lý ảnh của 2 model tự huấn luyện (loại xe, kiểu dáng, dùng `torchvision.transforms`) và giải mã CTC của OCR vẫn còn phụ thuộc torch cho các phép toán đơn giản (resize/chuẩn hóa, argmax); muốn bỏ hẳn torch khỏi toàn bộ pipeline cần viết lại thêm 2 chỗ này bằng numpy/cv2 thuần, để dành cho khi thật sự cần triển khai một thiết bị không cài được torch.
...
- Tích hợp cả 2 bước vào pipeline thật dùng chung cho backend và edge worker (`onnx_pipeline.py`), phát hiện và sửa 2 lỗi hiệu năng/tích hợp: model định vị bị nạp lại từ đĩa mỗi khung hình (135ms → 12,6ms sau khi cache), và tranh chấp luồng CPU giữa nhiều model chạy đồng thời (780-900ms/ảnh, dao động mạnh → ổn định 146,8ms/ảnh, xuống 76,2ms trên máy nhiều lõi qua biến môi trường `ML_INTRAOP_THREADS`). Mọi số hiệu năng đều sinh từ script `benchmark_pipeline.py` đã commit, chạy lại được để đối chiếu. Trong lúc tích hợp cũng phát hiện và sửa lỗi tên nhãn `motorcycle`/`motorbike` khiến xe máy không được gán nhóm phí tự động.
