...
Nhóm đã tìm và xem qua 14 dataset công khai, chia làm 3 nhóm: (A) biển số Việt Nam, (B) phương tiện/bối cảnh đường phố Việt Nam, (C) các dataset ALPR/bãi xe nước ngoài để đối chiếu. Mỗi dataset được kiểm tra trực tiếp trên trang nguồn (license, mô tả, ngày kiểm tra) trước khi đưa vào bảng so sánh dưới đây.
...
Không dataset nào ở nhóm A ghi rõ điều kiện ánh sáng hay bối cảnh chụp trong mô tả. A1 là dataset được ưu tiên vì có cả biển ngang và biển vuông, 2 loại biển phổ biến ở Việt Nam theo quy định.
...
Ở nhóm B, B1–B4 chưa có dataset Việt Nam nào tách phương tiện theo kiểu dáng (sedan/SUV/pickup), tất cả chỉ dừng ở mức "Car" chung. Nhóm tìm thêm B5 để bù chỗ thiếu này. Nhóm đã tải về và chạy EDA trên B5: 12 lớp cân bằng gần như tuyệt đối (chênh lệch max/min chỉ 1,02 lần), nhưng ảnh chủ yếu chụp kiểu dealer/showroom (tỉ lệ diện tích xe/ảnh trung bình 0,31, trong khi ảnh camera giám sát cổng bãi xe ở nhóm A/C chỉ khoảng 0,03–0,04) và bối cảnh gốc Trung Quốc (biển hiệu chữ Hán, theo nguồn CompCars). Khác biệt nằm ở kiểu ảnh chụp (studio/quảng cáo so với camera giám sát) và phân bố model xe của thị trường Trung Quốc so với Việt Nam. Hình dáng xe thì không khác nhiều, vì sedan/SUV/pickup thiết kế khá giống nhau giữa các thị trường.
Nhóm chọn dùng B5 trước để đánh giá tính khả quan của hướng pretrain trên B5 rồi fine-tune bằng ảnh thực tế Việt Nam (Nhật, Tuần 5); nếu không đáp ứng (ví dụ thiếu hẳn lớp motorbike, hoặc car-subtype lệch nhiều do phân bố model) sẽ bổ sung thêm dataset khác thay vì thay hẳn nguồn chính.
...
Nhóm C dùng biển số nước ngoài nên không dùng để train OCR biển VN được, nhưng CCPD (C1) có thể tham khảo để pretrain phần detect vùng biển số nói chung, vì đây là dataset duy nhất trong cả 3 nhóm có số liệu ngày/đêm rõ ràng, thứ mà cả nhóm A lẫn B đều thiếu.
...
Ban đầu, do trang Roboflow/Kaggle không mô tả gì về điều kiện chụp, nhóm nghĩ có thể phải tự chụp thêm gần như toàn bộ ảnh ở bối cảnh cổng bãi xe. Nhưng khi mở thử vài chục ảnh mẫu ra xem thì hóa ra cả 3 dataset đều là ảnh chụp thật từ camera cổng bãi xe/hầm gửi xe, có thanh chắn (boom barrier) trong khung hình, đúng bối cảnh đồ án cần chứ không phải ảnh đường phố chụp ngẫu nhiên như suy đoán ban đầu. Nhiều ảnh còn có timestamp cháy vào góc kiểu camera an ninh DVR, và có vài ảnh chụp trong hầm xe thiếu sáng. Mô tả trên trang dataset không đủ để đánh giá, phải tải về và xem ảnh thật mới biết chắc.
