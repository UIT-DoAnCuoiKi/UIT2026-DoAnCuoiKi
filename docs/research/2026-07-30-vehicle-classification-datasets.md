...
- **Sedan/SUV/hatchback/MPV (car subtype):** phủ tốt nhất về mặt danh sách nhãn (CompCars 12 lớp, VTID2 5 lớp, Vehicle Body Style Dataset 12 lớp), nhưng toàn bộ nguồn đều là ảnh xe Tây/Trung/Thái. Cần nói rõ rủi ro nằm ở đâu: hình dáng xe theo phân khúc (sedan gầm thấp/nắp capo dài, SUV gầm cao/nóc vuông...) là đặc điểm thiết kế khá phổ quát giữa các thị trường, nên bản thân việc phân loại thô sedan/SUV/hatchback/pickup không hẳn bị ảnh hưởng nhiều bởi việc ảnh chụp ở đâu. Hai rủi ro cụ thể hơn là: (a) phân bố model xe khác — các model phổ biến ở VN (Toyota Vios, Honda City, Kia Morning, Hyundai Accent...) có thể không được đại diện đầy đủ trong dữ liệu thiên về xe Mỹ/Trung/châu Âu; (b) kiểu ảnh khác — dataset quốc tế thường là ảnh chụp quảng cáo/dealer/showroom, khác hẳn ảnh camera giám sát thực tế tại cổng bãi xe (đã đo được bằng số ở mục G, không phải suy đoán).
...
- Xem ảnh mẫu (`eda_vbs_sample_by_class.png`, 3 ảnh/lớp × 12 lớp) thì thấy rõ hai điểm khác biệt so với bối cảnh triển khai thực tế — không phải khác biệt về hình dáng xe (sedan/SUV/hatchback... về thiết kế là phổ quát, không đổi theo quốc gia), mà là: (a) bối cảnh chụp: phần lớn ảnh có biển hiệu chữ Hán, biển số xe kiểu TQ ở vài ảnh, khung cảnh showroom/auto-show — đúng như nghĩ lúc khảo sát nguồn (dataset dựa theo CompCars, thu thập ở Trung Quốc), không có ảnh nào nhận ra là bối cảnh Việt Nam; (b) phân bố model xe theo thị trường Trung Quốc, có thể không đại diện đủ cho các model phổ biến tại VN.
...
- Ảnh chủ yếu là kiểu chụp "đẹp" dealer/auto-show/studio (nền sạch, góc chụp quảng cáo), khác hẳn góc camera giám sát cố định ở cổng bãi xe — thấy rõ qua `box_area_ratio` trung bình khá cao (0,31, phân bố hình chuông quanh 0,25–0,35) so với dataset biển số/bối cảnh bãi xe đã khảo sát trước đó (0,028–0,043). Đây là khác biệt lớn nhất và đo được rõ ràng nhất giữa dataset này và ảnh triển khai thật: khác kiểu ảnh (studio/quảng cáo vs surveillance), không phải khác hình dáng xe.
...
Nói chung dataset này hợp để làm nguồn pretrain/transfer learning cho việc phân biệt hình dạng xe (12 lớp cân bằng, rõ ràng) — vì hình dáng sedan/SUV/pickup/v.v. khá phổ quát nên không cần lo kiểu dáng "khác theo quốc gia". Nhưng nếu dùng thật thì vẫn cần fine-tune trên ảnh xe thực tế tại Việt Nam (chụp ở bãi xe/đường phố VN, model xe phổ biến VN) trước khi triển khai, chủ yếu để thích nghi với kiểu ảnh camera giám sát (khác hẳn ảnh dealer/showroom) và phân bố model xe theo thị trường VN — không nên dùng thẳng model train thuần trên dataset này. Điều này càng củng cố cho việc cần tự chụp bổ sung đã nói ở mục F, không chỉ riêng lớp motorbike mà cả car-subtype.

**Quyết định:** nhóm chọn dùng A1 (Vehicle Body Style Dataset) làm nguồn chính trước tiên để đánh giá tính khả quan của hướng pretrain + fine-tune này trên dữ liệu thực tế của đề tài (Tuần 5). Nếu sau khi fine-tune trên ảnh tự chụp/VN mà kết quả không đáp ứng — ví dụ car-subtype vẫn lệch nhiều do phân bố model, hoặc thiếu hẳn lớp motorbike — sẽ bổ sung thêm dataset khác theo hướng đã nêu ở mục E/F (CompCars A3 hoặc VTID2 A6 cho car-subtype, MIO-TCD B2 cho pickup/truck/bus, Vehicle Vietnam-CanTho hoặc tự chụp cho motorbike) thay vì thay hẳn nguồn chính.
